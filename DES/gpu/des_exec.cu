...
__global__ void paralell_enc_dec(const BYTE *data, BYTE *encrypted_data, BYTE *decrypted_data, int size)
{
...
void enc_dec_file(char filename[],int numThreads)
{
    ...
    int size = (int)(sizeof(BYTE) * st.st_size);

    h_encrypted_data = (BYTE *) malloc(size);
    h_decrypted_data = (BYTE *) malloc(size);
    ...
    err = cudaMalloc((void **)&d_encrypted_data, size);
    ...
    err = cudaMalloc((void **)&d_decrypted_data, size);
    ...
    fwrite(h_encrypted_data, size, 1, enc_file);
    fwrite(h_decrypted_data, size, 1, dec_file);

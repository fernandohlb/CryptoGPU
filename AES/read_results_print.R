# setwd("~/GitProjects/CryptoGPU/AES")
library("ggplot2")

aes_gpu_hubble_1 = read.table("./results_perf/aes_gpu/hubble_1.log", header = FALSE, sep = "\t", stringsAsFactors = FALSE)
...
#   facet_wrap(~imsize, scales = "free", labeller = "label_both") +
 ...
 pdf("resultado_aes.pdf", width = 16, height = 9)
 print(fig1)
 dev.off()
 ...
 fig2 <- ggplot(data=fulldata, aes(x = factor(filesize), y=time_elapsed, fill=type)) +
  ...
  write.csv(summarised_result, file = "./results_perf/summarised_result.csv")  
   ...
   pdf("resultado_aes_media.pdf", width = 16, height = 9)
   print(fig3)
   dev.off()

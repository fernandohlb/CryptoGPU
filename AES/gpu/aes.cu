...
// /**************************** HEADERS ****************************/
// __device__ void aes_decrypt(const BYTE in_all[], BYTE out_all[], const WORD key[], int keysize);
// __device__ void aes_encrypt(const BYTE in_all[], BYTE out_all[], const WORD key[], int keysize);
...
static const BYTE aes_sbox_host[16][16] = {
...
__device__ void xor_buf(const BYTE in[], BYTE out[], size_t len)
{
    size_t idx;
...
* AES - CBC
*******************/
__global__ void aes_encrypt_cbc(const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize, const BYTE iv[])
{
...
__global__ void aes_decrypt_cbc(const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize, const BYTE iv[])
{
    BYTE buf_in[AES_BLOCK_SIZE], buf_out[AES_BLOCK_SIZE], iv_buf[AES_BLOCK_SIZE];
    int blocks, idx;
...
* AES - ECB
*******************/
__global__ void aes_encrypt_ecb(const BYTE in_all[], size_t in_len, BYTE out_all[], const WORD key[], int keysize)
{
    BYTE in[AES_BLOCK_SIZE];
    BYTE out[AES_BLOCK_SIZE];
    int i = (blockDim.x * blockIdx.x + threadIdx.x) * AES_BLOCK_SIZE;
    if(i < in_len){
        for(int j = 0, temp = i; j < AES_BLOCK_SIZE; j++, temp++){
            in[j] = in_all[temp];
        }
...
__global__ void aes_decrypt_ecb(const BYTE in_all[], size_t in_len, BYTE out_all[], const WORD key[], int keysize)
{
    BYTE in[AES_BLOCK_SIZE];
    BYTE out[AES_BLOCK_SIZE];
    int i = (blockDim.x * blockIdx.x + threadIdx.x) * AES_BLOCK_SIZE;
...
WORD SubWord(WORD word)
{
    ...
    result = (int)aes_sbox_host[(word >> 4) & 0x0000000F][word & 0x0000000F];
    result += (int)aes_sbox_host[(word >> 12) & 0x0000000F][(word >> 8) & 0x0000000F] << 8;
    result += (int)aes_sbox_host[(word >> 20) & 0x0000000F][(word >> 16) & 0x0000000F] << 16;
    result += (int)aes_sbox_host[(word >> 28) & 0x0000000F][(word >> 24) & 0x0000000F] << 24;
    return(result);
}
